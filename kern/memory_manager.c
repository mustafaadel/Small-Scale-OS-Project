...
void * create_page_table(uint32 *ptr_page_directory,
	...
	uint32* ptr_page_table = (uint32*) kmalloc(PAGE_SIZE);
	if (ptr_page_table == NULL) {
		return NULL;
	}
//    a.    clear all entries (as it may contain garbage data)
	memset((uint32*) ptr_page_table, 0, PAGE_SIZE);
	uint32 Pa_page_table = kheap_physical_address(
			(unsigned int) ptr_page_table);

	//ptr_page_directory[PDX(virtual_address)]=Pa_page_table;
	ptr_page_directory[PDX(virtual_address)] = CONSTRUCT_ENTRY(
			(uint32 )Pa_page_table, PERM_PRESENT | PERM_USER | PERM_WRITEABLE);
	// ptr_page_directory[PDX(virtual_address)] =CONSTRUCT_ENTRY((uint32)Pa_page_table ,PERM_PRESENT | PERM_USER | PERM_WRITEABLE);
//ptr_page_directory[PDX(virtual_address)]=CONSTRUCT_ENTRY(Pa_page_table ,Pa_page_table|PERM_USER|PERM_PRESENT| PERM_WRITEABLE);
...
//panic("allocateMem() is not implemented yet...!!");
	...
	int allocated_pages = size / PAGE_SIZE;
	for (int i = 0; i < allocated_pages; i++) {
		int ret = pf_add_empty_env_page(e, virtual_address, 1);
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size) { //remove pages from pagefile
	...
	uint32 maximumWs_Size = e->page_WS_max_size;
	uint32 va;
	for (uint32 j = 0; j < maximumWs_Size; j++) {
		va = env_page_ws_get_virtual_address(e, j);
	...
	while (i < pages_no) {
		int flag = 1;
		pageTable = NULL;
		get_page_table(e->env_page_directory, (void*) startaddress, &pageTable);
		if (pageTable != NULL) {
			int j = 0;
			while (j < 1024) {
				if ((pageTable[j] & PERM_PRESENT) == PERM_PRESENT) {
					flag = 0;
					break;
				}
				j++;
			...
			if (flag == 1) {
				...
				pd_clear_page_dir_entry(e, (uint32) startaddress);
			}
		}
