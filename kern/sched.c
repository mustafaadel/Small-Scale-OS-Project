...
void sched_init_MLFQ(uint8 numOfLevels, uint8 *quantumOfEachLevel) {
	...
	for (uint32 j = 0; j < num_of_ready_queues; j++) {
		quantums[j] = quantumOfEachLevel[j];
	}
...
struct Env* fos_scheduler_MLFQ() {
	...
	while (i < num_of_ready_queues) {
		...
		//Else, return NULL
		s = queue_size(&env_ready_queues[i]);
		if (s > 0) {
			struct Env* environment = dequeue(&env_ready_queues[i]);
			kclock_set_quantum(quantums[i]);
			if (i <= num_of_ready_queues - 2)
				x = i + 1;

			else
				x = num_of_ready_queues - 1;
