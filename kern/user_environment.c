...
void env_free(struct Env *e) {
	uint32 TheEndOfUserStack = USER_TOP;
	struct Frame_Info *frame;
	uint32 permBuffered;
	uint32 permPressent;
	for (int i = 0; i < TheEndOfUserStack; i += PAGE_SIZE)
	{
		uint32 *pageTable = NULL;
		uint32 Permissions = pt_get_page_permissions(e, i);
		frame = get_frame_info(e->env_page_directory, (void*) i, &pageTable);
		permBuffered = Permissions & PERM_BUFFERED;
		permPressent = Permissions & PERM_PRESENT;
		if (permBuffered) {
			free_frame(frame);
	...
	for (int WS_idx = 0; WS_idx < e->page_WS_max_size; ++WS_idx) {
		env_page_ws_clear_entry(e, WS_idx);
	...
	while (k < USER_TOP) {
		uint32 *pagetable = NULL;
		get_page_table(e->env_page_directory, (void*) k, &pagetable);
		if (pagetable) {
			struct Frame_Info *ptr_frame_info = to_frame_info(
					kheap_physical_address((uint32) pagetable));
			free_frame(ptr_frame_info);
		}
		k += PAGE_SIZE * 1024;
	}
	struct Frame_Info *ptr_frame_info2 = to_frame_info(e->env_cr3);
