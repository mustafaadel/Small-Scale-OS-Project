...
void placement(struct Env * curenv, uint32 fault_va, uint32 replacement_index,uint32 replacement) {
	...
	int allocatedFrame = allocate_frame(&frame);
	if (allocatedFrame == E_NO_MEM) {
		return panic("MEMORY FULL");
	}
	int mappedFrame = map_frame(curenv->env_page_directory, frame,
			(void*) fault_va, page_permissions);
	if (mappedFrame == E_NO_MEM) {
		return panic("MEMORY FULL");
	}
	int pageRead = pf_read_env_page(curenv, (void*) fault_va);
	if (pageRead == E_PAGE_NOT_EXIST_IN_PF) {
		if ((fault_va > USTACKBOTTOM) && (fault_va <= USTACKTOP)) {
			pf_add_empty_env_page(curenv, fault_va, 1);
		} else {
			panic("The address is not in the stack %x \n", fault_va);
		}
	}
	if (replacement == 0) {
		uint32 i = curenv->page_last_WS_index;
	//	cprintf("index bara : %d\n", i);
		uint32 working_set_size = curenv->page_WS_max_size;
		while (working_set_size > 0) {
			i = i % (curenv->page_WS_max_size);
		//	cprintf("index gowa : %d\n", i);
		...
		//cprintf("index fl akher : %d\n", i);
		env_page_ws_set_entry(curenv, i, fault_va);
	} else {
		//cprintf("index else : %d\n", replacement_index);
		env_page_ws_set_entry(curenv, replacement_index, fault_va);
	}
}

void page_fault_handler(struct Env * curenv, uint32 fault_va) {
	uint32 theRealWS_Size = env_page_ws_get_size(curenv);
	uint32 maximumsize = curenv->page_WS_max_size;
	if (theRealWS_Size < maximumsize) {
		...
		if (isPageReplacmentAlgorithmModifiedCLOCK()) {
			...
			while (1) {
				...
				//cprintf("starting_index :%d \n", starting_index);
				for (int m = 0, i = starting_index;m < curenv->page_WS_max_size; m++, i++) {
					i = (i % curenv->page_WS_max_size);
					//cprintf("i :%d \n", i);
				...
				///cprintf("starting_index try one :%d \n", starting_index);
				for (int k = 0, i = starting_index;k < curenv->page_WS_max_size; k++, i++) {
					i = (i % curenv->page_WS_max_size);
					//cprintf("i try one:%d \n", i);
					uint32 temp_VA = env_page_ws_get_virtual_address(curenv, i);
					...
					if ((page_permissions & PERM_USED) == 0) {
						victim_VA = temp_VA;
						victim_index = i;
						//pt_set_page_permissions(curenv, victim_VA, PERM_MODIFIED, 0);
						curenv->page_last_WS_index = ((i + 1)% curenv->page_WS_max_size);
						flag = 0;
						if (flag == 0){
							//cprintf("try 1 i : %d",i);
		...
		pt_set_page_permissions(curenv, victim_VA, 0, PERM_PRESENT);
		uint32 page_permissions = pt_get_page_permissions(curenv, victim_VA);
		if (page_permissions & PERM_MODIFIED) {
			pf_update_env_page(curenv, (void*) victim_VA, frame_info);
